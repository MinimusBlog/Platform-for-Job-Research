<template>
  <div
    class="dashboard min-h-screen bg-bg text-white font-sans selection:bg-mint selection:text-bg pb-20"
  >
    <header
      class="relative z-10 flex items-center justify-between px-10 py-5 border-b border-border bg-bg-card/50 backdrop-blur-md"
    >
      <div class="flex items-center gap-12">
        ...
        <nav class="flex gap-8 text-[10px] font-bold uppercase tracking-[0.25em] text-gray-400">
          <router-link :to="{ name: 'company-profile' }" class="hover:text-white transition-colors"
      ...
      <div class="flex items-center gap-6 text-gray-400">
        ...
        <div
          class="w-8 h-8 rounded-lg bg-bg-elevated border border-border flex items-center justify-center text-[10px] font-bold text-white"
        >
          HR
        </div>
      </div>
    ...
    <main class="max-w-5xl mx-auto pt-16 px-6">
      ...
      <div class="flex justify-between items-end mb-12">
        ...
        <div class="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500">
          Шаг 1 из 3
        </div>
      </div>
      ...
      <section class="mb-12">
        <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-6 block"
          >— Тип публикации</label
        >
          ...
          <div
            ...
            :class="
              selectedType === type.id
                ? 'border-mint bg-mint/5 shadow-[0_0_20px_rgba(0,229,160,0.05)]'
                : 'border-border bg-bg-card/30 hover:border-gray-600'
            "
            class="p-6 rounded-xl border cursor-pointer transition-all group"
          >
            <div
              class="text-2xl mb-4 group-hover:scale-110 transition-transform"
              :class="selectedType === type.id ? 'opacity-100' : 'opacity-40'"
            >
              {{ type.icon }}
            </div>
      ...
      <div class="grid grid-cols-12 gap-10">
        ...
        <div class="col-span-7 space-y-10">
          <div>
            <label class="text-[9px] font-black uppercase tracking-widest text-gray-500 mb-3 block"
              >Название позиции</label
            >
            <input
              type="text"
              placeholder="например, Junior Frontend Developer (React)"
          ...
          <div>
            ...
            >
            <div
              class="border border-border rounded-xl bg-bg-card/20 overflow-hidden focus-within:border-mint/50 transition-colors"
            >
              ...
              <textarea
                placeholder="Расскажите о задачах, команде и стеке технологий..."
                class="w-full bg-transparent p-5 h-48 focus:outline-none resize-none text-sm leading-relaxed"
              ></textarea>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-6">
            <div>
              <label
                class="text-[9px] font-black uppercase tracking-widest text-gray-500 mb-3 block"
                >Навыки</label
              >
              <div
                class="flex flex-wrap gap-2 p-3 border border-border rounded-xl bg-bg-card/20 min-h-[46px]"
              >
                <span
                  class="bg-mint/10 text-mint text-[10px] px-2 py-1 rounded border border-mint/20 flex items-center gap-2"
                >
                ...
                <input
                  ...
                  class="bg-transparent text-[10px] outline-none w-20"
                />
              </div>
            </div>
            <div>
              ...
              >
              <select
                class="w-full bg-bg-card/20 border border-border rounded-xl p-3 text-xs font-bold focus:border-mint outline-none appearance-none cursor-pointer"
              >
        ...
        <div class="col-span-5 space-y-10">
          <div>
            ...
            <div class="relative mb-4">
              ...
              <input
                type="text"
                placeholder="Москва, БЦ 'Академик'"
            ...
            <!-- Плейсхолдер карты -->
            <div
              class="w-full h-32 rounded-xl bg-bg-elevated border border-border flex flex-col items-center justify-center relative overflow-hidden group"
            >
              <div class="absolute inset-0 bg-gradient-to-br from-mint/5 to-transparent"></div>
              <div
                class="text-[10px] font-bold text-gray-500 group-hover:text-mint transition-colors"
              >
                ● ПРЕДПРОСМОТР КАРТЫ
              </div>
            </div>
          ...
          <div>
            ...
            <div class="relative h-6 flex items-center">
              ...
              <div
                class="absolute left-1/4 right-1/4 h-[2px] bg-mint shadow-[0_0_10px_rgba(0,229,160,0.3)]"
              ></div>
              <div
                class="absolute left-1/4 w-3 h-3 bg-mint rounded-full -translate-x-1/2 shadow-lg"
              ></div>
              <div
                class="absolute right-1/4 w-3 h-3 bg-mint rounded-full translate-x-1/2 shadow-lg"
              ></div>
            </div>
            <div
              class="flex justify-between mt-2 text-[8px] font-black uppercase text-gray-600 tracking-tighter"
            >
              <span>40K</span>
              <span>500K+</span>
            </div>
          ...
          <div class="grid grid-cols-2 gap-6">
            <div>
              ...
              <input
                type="date"
                class="w-full bg-bg-card/20 border border-border rounded-xl p-3 text-xs font-bold focus:border-mint outline-none invert-[0.8] brightness-150"
              />
            </div>
            <div>
              <label
      ...
      <div class="mt-20 flex justify-end gap-4 border-t border-border pt-10">
        <button class="btn-outline px-10 py-3 text-[11px] uppercase tracking-[0.2em]">
          Предпросмотр
        </button>
        <button
          class="btn-primary px-12 py-3 text-[11px] uppercase tracking-[0.2em] shadow-[0_0_30px_rgba(0,229,160,0.2)]"
        >
          Продолжить
        </button>
      </div>
    ...
    <!-- Футер из дизайна -->
    <footer
      class="mt-20 px-10 py-10 border-t border-white/5 flex justify-between items-center opacity-40"
    >
      <div class="space-y-2">
        <div class="text-[11px] font-black tracking-[0.4em] text-mint uppercase">ТРАМПЛИН</div>
        <div class="text-[9px] font-bold text-gray-500 tracking-widest uppercase">
